...
export class Game_Object{
    constructor(spawn_time){
        ...
        let spawn_locations = [[-4,-4,0], [-3,-4,0], [0, -4, 0], [0, -4, 0], [2,-4,0], [4,-4,0]];
        let spawn_velocities =  [[1.5,12], [2,11],[-1,9], [0, 12], [2, 10], [-3,15]];
        ...
        this.spawn_velocity = spawn_velocities[Math.floor(Math.random() * 6)]
    ...
    update_state(time, scene = 0){
        ...
        this.projectile_transform = this.model_transform.times(Mat4.translation(this.spawn_location[0], this.spawn_location[1], this.spawn_location[2]))
                                                        .times(Mat4.translation(this.vx*t2, this.vy*t2-4.9*t2**2, 0)).times(Mat4.scale(this.scale,this.scale,this.scale))
